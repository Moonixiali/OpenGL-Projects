...
void App::run() {
    ...
    while (!glfwWindowShouldClose(window)) {
		...
		if (elapTime >= 1.0f/fpsLimit) {
			...
			motionSystem->update(
				transformComponents, physicsComponents, tFromFrame);
			bool should_close = cameraSystem->update(
				transformComponents, cameraID, *cameraComponent, tFromFrame, speed);
			if (should_close) {
				break;
			}
			animationSystem->update(animationComponents, tFromFrame * 1000);
